...
def test_private_store_detail_premium_sections_privacy_and_profile_image_route():
    ...
    assert "ps-detail-photo-img" in html
    assert "ps-detail-photo-frame" in html
    assert "ps-card-photo" not in html
...
def test_private_store_interview_protected_access_and_redaction():
    ...
    assert "resumen de entrevista" not in html
    assert "datos generales" not in html
...
def test_private_store_detail_shows_protected_button_with_structured_interview():
    ...
    assert "datos generales" not in iv_html
    assert "resumen de entrevista" not in iv_html
